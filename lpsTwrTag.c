...
float ftround1, ftreply1, ftreply2, ftround2, ftprop_ctn;
...
static uint32_t rxcallback(dwDevice_t *dev) {
  ...
  switch(rxPacket.payload[LPS_TWR_TYPE]) {
    ...
    case LPS_TWR_REPORT:
    {
      ...
      if(User_count%150==0){
    	  times++;
      DEBUG_PRINT("The %d count:\n",times);
      DEBUG_PRINT("troudn1:%f\n",tround1);
      DEBUG_PRINT("treply1:%f\n",treply1);
      DEBUG_PRINT("tround2:%f\n",tround2);
      DEBUG_PRINT("treply2:%f\n",treply2);
      User_count=0;
      }//DEBUG_PRINT("tprop_ctn:%f",tprop_ctn);
      User_count++;
      ftround1=(float)tround1;
      ftround2=(float)tround2;
      ftreply1=(float)treply1;
      ftreply2=(float)treply2;
